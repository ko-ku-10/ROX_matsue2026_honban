...
camera_lateral_offset_m = 1

# カメラがロボットの真正面からどれだけ回転して付いているか[度]。
# ロボットをTagへ真正面に置いた時に、サイトの「画像方位角」が +3.0°なら、
# ここへ -3.0 を入れる。補正後の角度0°をロボット真正面として扱う。
camera_yaw_offset_deg = 0.0

# 魚眼カメラの左右端を、ロボットから見て何度横だとみなすか[度]。
# 90.0なら、画面の右端=右真横(+90°)、左端=左真横(-90°)として旋回する。
# 魚眼の端ではsolvePnPのx/zを信用せず、この画像上の方位角で中央へ向ける。
camera_edge_bearing_deg = 90.0
