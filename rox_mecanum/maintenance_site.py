...
<section class=card style="margin-top:16px"><h2>AUTO DECISION TRACE <span class=muted>（GAME2の判断根拠）</span></h2><div id=autoDebug class="tags"><span class=empty>自動照準を待っています…</span></div></section>
<section class=card style="margin-top:16px"><h2>AIMING MAP <span class=muted>（カメラ基準の擬似地図・上が前方）</span></h2><svg id=aimMap class=aim-map viewBox="0 0 800 365" role="img" aria-label="自動照準の擬似地図"></svg><div id=mapNote class=map-note>Tagの位置を待っています…</div></section>
...
function putStick(id,magId,dotId,v){let x=v?Math.max(-1,Math.min(1,v.x)):0;let y=v?Math.max(-1,Math.min(1,v.y)):0;$(id).textContent=v?'x '+n(x)+' / y '+n(y):'--';$(magId).textContent='magnitude '+(v?n(v.magnitude):'--');$(dotId).style.transform='translate(calc(-50% + '+(x*34)+'px), calc(-50% + '+(-y*34)+'px))'}
function renderAimMap(tags,dbg){
  let points=Object.entries(tags).filter(([,t])=>Number.isFinite(t.lateral_m)&&Number.isFinite(t.forward_m)&&t.forward_m>0);
  let selected=String(dbg['目標Tag']||'').match(/\\d+/g)||[];
  let targetZ=Number(dbg['目標 z[m]']);
  let maxZ=Math.max(2,...points.map(([,t])=>t.forward_m),Number.isFinite(targetZ)?targetZ:0)*1.15;
  let maxX=Math.max(1,...points.map(([,t])=>Math.abs(t.lateral_m)))*1.35;
  let sx=x=>400+x/maxX*330,sy=z=>330-z/maxZ*280;
  let grid='';
  for(let i=-1;i<=1;i++){let x=sx(i*maxX/2);grid+='<line x1="'+x+'" y1="25" x2="'+x+'" y2="330" stroke="#1e4b67" stroke-dasharray="4 5"/><text x="'+x+'" y="353" text-anchor="middle" fill="#7da2bd" font-size="11">'+(i*maxX/2).toFixed(1)+'m</text>'}
  for(let i=1;i<=3;i++){let z=maxZ*i/3,y=sy(z);grid+='<line x1="45" y1="'+y+'" x2="755" y2="'+y+'" stroke="#1e4b67" stroke-dasharray="4 5"/><text x="48" y="'+(y-4)+'" fill="#7da2bd" font-size="11">z '+z.toFixed(1)+'m</text>'}
  let goal=Number.isFinite(targetZ)?'<line x1="45" y1="'+sy(targetZ)+'" x2="755" y2="'+sy(targetZ)+'" stroke="#bd7aff" stroke-width="2" stroke-dasharray="8 5"/><text x="755" y="'+(sy(targetZ)-6)+'" text-anchor="end" fill="#d6a6ff" font-size="12">設定距離 '+targetZ.toFixed(2)+'m</text>':'';
  let dots=points.map(([id,t])=>{let chosen=selected.includes(id),color=chosen?'#d18bff':t.fresh?'#37e79b':'#ffbd61';return '<g><circle cx="'+sx(t.lateral_m)+'" cy="'+sy(t.forward_m)+'" r="'+(chosen?14:11)+'" fill="'+color+'" stroke="#e9f8ff" stroke-width="2"/><text x="'+sx(t.lateral_m)+'" y="'+(sy(t.forward_m)+4)+'" text-anchor="middle" fill="#07111f" font-size="11" font-weight="800">'+id+'</text><text x="'+sx(t.lateral_m)+'" y="'+(sy(t.forward_m)-18)+'" text-anchor="middle" fill="'+color+'" font-size="11">x '+t.lateral_m.toFixed(2)+' / z '+t.forward_m.toFixed(2)+'</text></g>'}).join('');
  let forward=Number(dbg['最終 前後'])||0,strafe=Number(dbg['最終 横'])||0,rotate=Number(dbg['最終 旋回'])||0;
  let command='';
  if(Math.hypot(forward,strafe)>=.01){let size=Math.min(115,35+Math.hypot(forward,strafe)*90),dx=strafe*size,dy=-forward*size;command='<defs><marker id="moveArrow" markerWidth="9" markerHeight="9" refX="7" refY="4.5" orient="auto"><path d="M0,0 L9,4.5 L0,9 Z" fill="#3ee5ff"/></marker></defs><line x1="400" y1="316" x2="'+(400+dx)+'" y2="'+(316+dy)+'" stroke="#3ee5ff" stroke-width="7" stroke-linecap="round" marker-end="url(#moveArrow)"/><text x="400" y="275" text-anchor="middle" fill="#75edff" font-size="13">移動指令 前後 '+forward.toFixed(2)+' / 横 '+strafe.toFixed(2)+'</text>'}
  if(Math.abs(rotate)>=.01){let direction=rotate>0?'↻':'↺';command+='<text x="432" y="313" fill="#ffbd61" font-size="28" font-weight="800">'+direction+'</text><text x="452" y="310" fill="#ffcf88" font-size="12">旋回 '+rotate.toFixed(2)+'</text>'}
  let robot='<g><path d="M400 302 L384 330 L416 330 Z" fill="#3ee5ff" stroke="#e7faff" stroke-width="2"/><text x="400" y="350" text-anchor="middle" fill="#bfefff" font-size="12">ROBOT (x=0, z=0)</text></g>';
  $("aimMap").innerHTML=grid+goal+dots+command+robot;
  $("mapNote").textContent=points.length?'緑: 検出Tag / 紫: 現在の狙い / 破線: 設定距離 / 水色矢印: 最終移動指令 / オレンジ: 旋回指令。':'位置x/zが取れるTagを待っています…';
}
function render(v){let good=v.running!==false;$("dot").className='dot '+(good?'ok':'bad');$("liveText").textContent=good?'LIVE / 更新中':'STOPPED';$("game").textContent=v.game||'ROX MAINTENANCE';$("stage").textContent=v.stage||v.message||'状態を待っています';$("mode").textContent='モード: '+(v.mode||'--');let c=v.controller||{};let bs=c.active_buttons||[];$("buttons").innerHTML=allButtons.map(x=>'<span class="button-name '+(bs.includes(x)?'on':'off')+'">'+x+'</span>').join('');putStick('leftStick','leftMagnitude','leftDot',c.left_stick);putStick('rightStick','rightMagnitude','rightDot',c.right_stick);let sv=v.servos||{};putServo('catchServo','catch',sv.catch);putServo('liftServo','lift',sv.lift);$("pidError").textContent=sv.pid_error?'PID ERROR: '+sv.pid_error:'PID: 正常';let cm=v.camera||{};$("cameraNote").textContent=cm.connected===false?'CAMERA ERROR: '+(cm.error||'接続できません'):'カメラ接続中 / Tagの枠と番号を映像に重ねて表示';let wheels=(v.mecanum||{}).wheel_speed_commands||{};let a=(v.mecanum||{}).acceleration_limit_per_sec;$("wheels").innerHTML=['FL','FR','RL','RR'].map(x=>'<div class=metric><b>'+x+'</b><strong>'+n(wheels[x],3)+'</strong><small>加速制限 '+n(a,2)+'/s</small></div>').join('');let dbg=v.auto_debug||{};let debugEntries=Object.entries(dbg);$("autoDebug").innerHTML=debugEntries.length?debugEntries.map(([key,value])=>'<div class=metric><b>'+key+'</b><strong>'+((typeof value==='number')?n(value,3):(value===true?'はい':value===false?'いいえ':String(value)))+'</strong></div>').join(''):'<span class=empty>自動照準を待っています…</span>';let tags=v.tags||{};renderAimMap(tags,dbg);let entries=Object.entries(tags);$("tags").innerHTML=entries.length?entries.map(([id,t])=>'<div class=tag><div class=tag-head><span>TAG '+id+'</span><span class='+(t.fresh?'fresh':'old')+'>'+ (t.fresh?'LIVE':'OLD')+'</span></div><small>位置 x '+n(t.lateral_m,3)+' m / z '+n(t.forward_m,3)+' m</small><small>距離 '+n(t.distance_m,3)+' m</small><small>左右ずれ '+n(t.robot_x_error,3)+'</small><small>Tag角度 '+n(t.yaw_degrees,1)+'°</small><small>受信 '+n(t.age_sec,3)+' 秒前</small></div>').join(''):'<span class=empty>Tagを待っています…</span>';$("controls").hidden=!v.controls_available;$("raw").textContent=JSON.stringify(v,null,2)}
